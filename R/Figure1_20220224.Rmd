...
### combined Ctrl-swim vs treatments strength (FigS1B-D)

```{r}
cmb <- intersect(intersect(rownames(df_swvsyoh),rownames(df_swvswpro)),rownames(df_swvswpra))
 FigS1BD <- ggplot() + geom_point(data = df_swvsyoh[cmb,],aes(index, sw), color = "blue") + 
   geom_point(data = df_swvsyoh[cmb,],aes(index, yoh), color = "red") + 
   geom_point(data = df_swvswpro[cmb,],aes(index, sw_pro), color = "green") + 
   geom_point(data = df_swvswpra[cmb,],aes(index, sw_pra), color = "orange") + 
   theme_bw() + 
   ylab("logFC") + 
...
pdf(file = "../Figures/FigS1BD.pdf", width = 4, height = 4)
print(FigS1BD)
dev.off()
...
saveRDS(STR2,"../final_data/STR2_Complete2.SE.Rds")
